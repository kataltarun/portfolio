...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50">
        <div className="flex justify-between items-center max-w-6xl mx-auto md:my-2 bg-stone-950/30 p-4 md:rounded-xl baackdrop-blur-lg ">
        <div className="uppercase text-lg font-semibold text-white">
            <a href="/">Tarun Katal</a>
        </div>
        ...
        <div className="md:hidden">
            <button onClick={()=> setMenuOpen(!menuOpen)} className="text-white focus:outline-none" aria-label={menuOpen? "Close menu" : "Open menu"} >
                {menuOpen ? (<RiCloseFill className="size-6"/>) : (<RiMenu3Fill className="size-6"/>)}
            </button>
        </div>
        </div>
        {menuOpen && (
