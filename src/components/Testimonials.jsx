...
const Testimonials = () => {
  ...
  return (
    <section className="px-6 py-10 min-h-screen " id="testimonials">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:grid-cols-3">{TESTIMONIALS.map((testimonial,index)=>(<motion.div
            key={index} className="relative rounded-lg p-6 h-full backdrop-blur-3xl bg-stone-500/10"   custom={index} initial="hidden" whileInView="visible" variants={childVariants}>
                ...
                <p className="loading-relaxed">{testimonial.feedback}</p>
        </motion.div>))}</div>
    </section>
  )
}
