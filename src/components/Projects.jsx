...
const Projects = () => {
    const projectVariants ={
        ...
        visible: {opacity: 1, scale: 1, rotate: 0, y: 0, transition: {duration: 0.6, ease: "easeOut", type: "spring", bounce: 0.4 } }
    }
  return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-6">
            {PROJECTS.map((project,index)=>(
                <motion.div key={index} className="relative rounded-lg overflow-hidden h-[500px] transition transform" initial="hidden" whileInView="visible" viewport={{once:true}} variants={projectVariants}>
                    <img src={project.image}  alt={project.name} className="inset-0 size-full object-cover transition-opacity absolute duration-300"/>
                    <div className="relative z-20 p-6 flex flex-col justify-between h-full bg-black/30 text-white">
                        ...
                        <div className="flex flex-col justify-between">
                            ...
                            <a href={project.link} target="_blank" rel="noopener norefferer" className="bg-white text-stone-900 rounded-full py-2 px-2 w-32 text-sm text-center hover:bg-gray-100">View on Github</a>
                        </div>
                    </div>
                </motion.div>
            ))}
